const { is_accidental } = require('../../src/services/interactionService');
...
describe('Interaction Service — is_accidental', () => {
  test('TC-IT-01: dentro del umbral', () => {
    ...
    const reverted = 4999;
    const TH = 5000;
    expect(is_accidental(created, reverted, TH)).toBe(true);
  });
  ...
  test('TC-IT-02: exactamente en el límite', () => {
    ...
    const reverted = 5000;
    const TH = 5000;
    expect(is_accidental(created, reverted, TH)).toBe(false);
  });

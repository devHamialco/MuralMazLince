...
const registerStudent = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ message: 'Estudiante registrado', userId: result.rows[0].id });
  } catch (error) {
    if (error.code === '23505') {
...
const registerEntrepreneur = async (req, res) => {
  const { matricula, password, whatsapp_number, display_name, privacy_accepted } = req.body;
  ...
  try {
    ...
    const userResult = await client.query(
      ...
      [matricula, hashedPw, whatsapp_number || null]
    );
    ...
    res.status(201).json({ message: 'Emprendedor registrado', userId });
  } catch (error) {
    await client.query('ROLLBACK');
    if (error.code === '23505' || error.message === 'DUPLICATE_MATRICULA') {
      return res.status(409).json({ error: 'La matrícula ya está registrada' });
    }
    console.error('Registration Error:', error);
    res.status(500).json({ error: 'Error interno del servidor en transacción' });
  } finally {
    client.release();
  }
};
...
const claimMatricula = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ message: 'Reclamo ingresado para su revisión', ticketId: result.rows[0].id });
  } catch (err) {
    res.status(500).json({ error: 'Error del servidor al ingresar reclamo' });
  }
};
...
<head>
  ...
  <style>
    ...
    h1 { color: #0b4d8c; }
    h2 { color: #1168BD; margin-top: 2rem; }
  </style>
</head>
<body>
  ...
  <p>La Universidad Tecmilenio (campus Mazatlán) a través del proyecto Mural Maz Lince, es la responsable del uso y protección de sus datos personales, en estricto apego a la Ley Federal de Protección de Datos Personales en Posesión de los Particulares (LFPDPPP).</p>
  
  <h2>2. Datos Recabados</h2>
  <p>Para la prestación de nuestros servicios de plataforma de comercio universitario, podemos recabar los siguientes datos personales: matrícula estudiantil, número de WhatsApp y nombre público (display name). Su contraseña es procesada de manera cifrada.</p>
  ...
  <ul>
    ...
    <li>Proveer un contacto protegido con los miembros de la red estudiantil a través de hipervínculos wa.me.</li>
